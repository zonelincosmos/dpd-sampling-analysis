...
每個 order k 的 basis function `x·|x|^(k-1)` 產生的頻寬是 k × BW（嚴格 hard cutoff，推導見 `PA_poly_regrowth_report.md` 第 3 節）。

如果 DPD 的取樣率 Fs = M × BW，那 Nyquist 頻率是 Fs/2 = M × BW/2。當 k > M 時，basis 的頻寬超過 Nyquist，超出的部分會**折回**（alias）。
...
| Fs_ref | 720 MHz (36X) | 參考取樣率，確保所有 K ≤ 9 都無 aliasing |
| N_ref | 98304 | 可被 D = [12, 6, 4, 3] 整除 |
...
| 係數 | 值 | 物理意義 |
|---|---|---|
| a₁ | 1.0 | 小訊號增益（linear gain） |
| a₃ | -0.3 + 0.1j | 3 階 intermodulation + AM/PM |
| a₅ | 0.08 - 0.05j | 5 階非線性 |
| a₇ | -0.02 + 0.01j | 7 階非線性（較弱） |
...
| Oversampling M = Fs / BW | 3, 6, 9, 12 |
| 對應 Fs | 60, 120, 180, 240 MHz |
| DPD 訓練方式 | Iterative multiplicative correction, 20 iterations |
| 正規化 λ | 10⁻⁶（Tikhonov regularization） |
...
在 Fs_ref = 720 MHz 計算所有 basis function。對 K = 9，basis bandwidth = 9 × 20 = 180 MHz，遠小於 Nyquist = 360 MHz，所以 reference 完全無 aliasing。

### 3.2 比較流程

對每個目標取樣率 Fs = M × BW：

1. **Decimate**：從 720 MHz 的 master signal 取每第 D 個 sample → 得到 Fs 下的訊號
2. **計算 basis**：在 Fs 計算 y_k = x·|x|^(k-1)（當 k > M，頻譜超出 Nyquist 會自動 alias）
...
**條件 1 — Total aliasing**（K > M）：basis 的頻寬 K × BW 超過 Fs，超出 Nyquist 的頻譜折回到 [-Fs/2, Fs/2] 範圍內。

**條件 2 — In-band aliasing**（K ≥ 2M）：折回的頻譜**剛好落在 signal band**（±BW/2 = ±10 MHz）。這比條件 1 更嚴格。

推導：頻率 f₀（介於 Fs/2 到 K·BW/2 之間）折回到 f₀ − Fs。要落在 signal band 內：
...
因為 f₀ 最大只到 K·BW/2，所以需要這個範圍和 [Fs/2, K·BW/2] 有交集：

```math
\frac{K \cdot \text{BW}}{2} > F_s - \frac{\text{BW}}{2} = \frac{(2M - 1) \cdot \text{BW}}{2}
```

化簡得 K > 2M − 1，因為 K 是整數，所以 **K ≥ 2M**。
...
- **OK** = 無 aliasing
- **Edge** = basis bandwidth 剛好等於 Fs，邊界值為零，不 alias
- **OOB-only** = aliasing 落在 adjacent channel（10-30 MHz），**不影響 signal band**
- **IN-BAND** = aliasing 落在 signal band（±10 MHz），**直接污染訊號**
...
### 4.4 In-Band NMSE（僅 ±10 MHz 內的 error）
...
K = 9 的 basis bandwidth = 9 × 20 = 180 MHz，頻譜範圍 ±90 MHz。
在 Fs = 60 MHz（M=3），Nyquist = 30 MHz。
...
以 K=9 DPD 為例，各 order 的係數大小 |w_k|（dB = 20·log₁₀|w_k|）：
...
Basis function `x·|x|^(k-1)` 的振幅隨 k 急遽下降。舉例：若訊號 RMS ≈ 0.36，那 |x|^(k-1) 的 RMS：
...
DPD 輸出中第 k 階的實際貢獻 ≈ |w_k| × RMS(x·|x|^(k-1))。即使 |w₉| 有 16.5 dB（≈ 6.7 倍），乘上 0.00028 的 basis RMS 後，貢獻只有 ≈ 0.002，遠小於 k=1 的 ≈ 0.97。
...
在實際 DPD 系統中，PA 輸出會經由 **feedback ADC**（回授類比數位轉換器）取樣，以 Fs 送回做 DPD 訓練。PA 的 output bandwidth 最大可到 7 × 20 = 140 MHz（因為 PA 是 7 階）。如果 feedback ADC 只有 60 MHz（M=3），PA output 中 30-140 MHz 的非線性產物會 alias。

| M | Fs (MHz) | Obs NMSE total | Obs NMSE in-band |
|---|---|---|---|
...
M=3 的 in-band observation aliasing 只有 -99.8 dB（因為 PA 的 7 階係數 a₇ = -0.02 + 0.01j 很弱），可忽略。M ≥ 6 時 observation aliasing 完全消失。
...
Condition number 主要取決於 basis function 的 dynamic range（|x|⁸ 和 |x|⁰ 的振幅比），**和取樣率幾乎無關**。K=9 的 cond ≈ 900，加上 Tikhonov regularization（λ = 10⁻⁶）後 LS 求解穩定。
...
| K ≥ 2M | In-band aliasing（折到 signal band） | Basis waveform 被污染 |
...
| 設計情境 | 建議最低 M | 理由 |
|---|---|---|
| DPD K ≤ 9，一般 PA | **M ≥ 3** | 3X 的 DPD 效能和 12X 差 < 0.5 dB |
| 強非線性 PA（a₅, a₇ 大） | **M ≥ 6** | 確保 in-band 零 aliasing |
| 需要 clean adjacent channel | **M ≥ K** | 避免 OOB aliasing |
| 需要 clean observation path | **M ≥ 6** | 對 7 階 PA |
